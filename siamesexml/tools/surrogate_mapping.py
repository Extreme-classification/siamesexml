import numpy as np
from xclib.utils.sparse import normalize, compute_centroid
import functools
...
class SurrogateMapping(object):
    ...
    def get_valid_labels(self, labels, label_features):
        freq = np.array(labels.sum(axis=0)).ravel()
        ind = np.where(freq > 0)[0]
        if label_features is not None:
            freq = np.array(label_features.sum(axis=1)).ravel()
            ind_1 = np.where(freq > 0)[0]
            ind = np.intersect1d(ind, ind_1)
        return labels[:, ind], ind

    def fit(self, features, labels, label_features):
        ...
        labels, self.valid_labels = self.get_valid_labels(labels, label_features)
        self.gen_mapping(features, labels)


def run(feat_fname, lbl_fname, lbl_feat_fname, feature_type, method, threshold, seed, tmp_dir):
    np.random.seed(seed)
    if feature_type == 'dense':
        features = data_utils.read_gen_dense(feat_fname)
        label_features = data_utils.read_gen_dense(lbl_feat_fname)
    elif feature_type == 'sparse':
        features = data_utils.read_gen_sparse(feat_fname)
        label_features = data_utils.read_gen_sparse(lbl_feat_fname)
    else:
        raise NotImplementedError()
    ...
    sd.fit(features, labels, label_features)
